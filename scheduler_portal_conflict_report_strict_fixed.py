...
# Load tide data from external CSVs
tide_data = {}
loaded_files = []
...
def schedule_customer(data):
    ...
    crane_ramp = st.session_state.crane_schedule.get(date.date())
    ...
    while cursor + duration <= end_dt:
        truck_day = st.session_state.truck_bookings[truck].get(date.date(), [])
        if has_conflict(truck_day, cursor, cursor + duration):
            cursor += timedelta(minutes=15)
            continue

        if requires_crane:
            crane_day = st.session_state.truck_bookings[17].get(date.date(), [])
            if has_conflict(crane_day, cursor, cursor + duration):
                cursor += timedelta(minutes=15)
                continue

        end_time = cursor + duration
        st.session_state.truck_bookings[truck].setdefault(date.date(), []).append((cursor, end_time))
        if requires_crane:
            st.session_state.truck_bookings[17].setdefault(date.date(), []).append((cursor, end_time))
            st.session_state.crane_schedule[date.date()] = ramp

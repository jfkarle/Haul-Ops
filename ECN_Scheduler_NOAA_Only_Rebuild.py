# Fetch live NOAA tides per station using mapped ramp names
def fetch_daytime_high_tides_2025_per_station(ramp_names):
    ...
    station_ids = set(get_station_for_ramp(r) for r in ramp_names)
    all_predictions = []
    ...
    for station_id in station_ids:
        ...
        try:
            ...
            for entry in response.json().get("predictions", []):
                t = datetime.strptime(entry["t"], "%Y-%m-%d %H:%M")
                if entry["type"] == "H" and datetime.strptime("07:30", "%H:%M").time() <= t.time() <= datetime.strptime("17:00", "%H:%M").time():
            ...
            print(f"Error loading NOAA tide data for {station_id}: {e}")
...
for harbor, filename in tide_files.items():
    url = f"# Removed GitHub URL tide fallback"

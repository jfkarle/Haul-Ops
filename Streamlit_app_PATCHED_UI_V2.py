...
def format_date_mmddyy(date_obj):
    """Formats a date object (date or datetime) to MM/DD/YY string."""
    if isinstance(date_obj, datetime):
        return date_obj.strftime("%m/%d/%y")
    elif isinstance(date_obj, date): # Handle date objects too
        return date_obj.strftime("%m/%d/%y")
...
# ------------- UI -------------------
# ====================================
st.markdown("<h1 style='font-size: 2rem;'>Boat Ramp Scheduling</h1>", unsafe_allow_html=True)
...
st.markdown("<h2 style='font-size: 1.5rem;'>Available Slots</h2>", unsafe_allow_html=True)
        ...
        if available_slots:
            ...
            for i, slot in enumerate(available_slots):
                with cols[i]:
                    ...
                    st.markdown(f"<div class='custom-time'><strong>Time:</strong> {slot['time'].strftime('%H:%M')}</div>", unsafe_allow_html=True)

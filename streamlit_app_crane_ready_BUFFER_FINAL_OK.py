...
def safe_pdf_output(pdf_obj):
    pdf_bytes = pdf_obj.output(dest='S').encode('latin-1', 'replace')
...
with st.sidebar:
    ...
    if st.session_state.ALL_JOBS:
        buffer = io.BytesIO()
        st.session_state.PDF_REPORT = FPDF()  # Re-init PDF for clean slate
        safe_pdf_output(st.session_state.PDF_REPORT)
        st.download_button(

...
with st.sidebar:
    ...
    # Display high tide time (if any) in the sidebar after user clicks
    if 'available_slots' in st.session_state and st.session_state['available_slots']:
        ht_time = st.session_state['available_slots'][0].get('high_tide')
        if ht_time:
            st.markdown(f"**High Tide on {format_date_display(st.session_state['available_slots'][0]['date'])}: {ht_time}**")

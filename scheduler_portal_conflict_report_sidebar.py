...
ramp_options = ['Sandwich Basin', 'Plymouth Harbor', 'Cordage Park (Ply)', 'Duxbury Harbor', 'Green Harbor (Taylors)', 'Safe Harbor (Green Harbor)', 'Ferry Street (Marshfield Yacht Club)', 'South River Yacht Yard', "Roht (A to Z / Mary's)", 'Scituate Harbor (Jericho Road)', 'Cohasset Harbor (Parker Ave)', 'Hull (A St, Sunset, Steamboat)', 'Hull (X Y Z St) (Goodwin V St)', 'Hingham Harbor', 'Weymouth Harbor (Wessagusset)']
...
def schedule_customer(data):
    ...
    while cursor + duration <= end_dt:
        ...
        conflict = any((s < cursor + duration and e > cursor) for s, e in truck_day_bookings)
        if not conflict:
            if requires_crane:
                crane_conflict = any((s < cursor + duration and e > cursor)
